...
describe('RulesView', () => {
  ...
  it('previews a draft without saving', async () => {
    ...
    render(RulesView, { onStartUpdate: vi.fn() });
    await fireEvent.click(await screen.findByRole('button', { name: '编辑' }));
    await fireEvent.click(screen.getByRole('tab', { name: '调试预览' }));
    ...
    await fireEvent.click(within(screen.getByRole('tablist', { name: '规则编辑' })).getByRole('tab', { name: '编辑' }));
...
it('groups sources by dragging and saves shared processing inside the group', async () => {
  ...
  const scriptSection = within(group).getByText('预处理 · 已配置').closest('details')!;
  expect(scriptSection.open).toBe(false);
  await fireEvent.click(within(group).getByText('预处理 · 已配置'));
...
it('edits shared preprocessing while preserving flat sources', async () => {
  ...
  render(RulesView, { onStartUpdate: vi.fn() });
  await fireEvent.click(await screen.findByRole('button', { name: '编辑' }));
  const processing = screen.getByText('统一预处理 · 已配置').closest('details')!;
  await fireEvent.click(within(processing).getByText('统一预处理 · 已配置'));
  const script = 'function process(content) { return content.trim(); }';
  await fireEvent.input(within(processing).getByLabelText('JavaScript'), { target: { value: script } });
  await fireEvent.click(screen.getByRole('button', { name: '保存规则' }));
  await screen.findByText('规则已保存');
