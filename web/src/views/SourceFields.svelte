...
<div class="fields">
  ...
  <label>备注<PixelInput {disabled} bind:value={source.label} placeholder="可选" /></label>
</div>
{#if source.kind === 'url'}
  <label class="stack">URL<PixelInput {disabled} bind:value={source.url} placeholder="https://" /></label>
...
{:else if source.kind === 'content'}
  <label class="stack">内联文本<PixelTextarea {disabled} bind:value={source.content} rows={6} placeholder="DOMAIN,example.com" /></label>
...
{:else if source.kind === 'geosite'}
  <div class="fields">
    ...
    <label>列表<PixelInput {disabled} bind:value={source.list} placeholder="google" /></label>
    <label class="full">属性<PixelInput {disabled} bind:value={source.attrs} placeholder="可选，例如 cn,ads" /></label>
  </div>
{:else}
  <div class="fields">
    <div class="stack">
    ...
    <label>列表<PixelInput {disabled} bind:value={source.list} placeholder="cn" /></label>
  </div>
{/if}
<style>
