<script lang="ts">
  ...
  import { api, APIRequestError, type GeositeEntry, type GeositeListOverview } from '../api/client';
  ...
  import { entryTypeLabel } from './geosite';
  ...
  let catalog = $state<GeositeListOverview[]>([]);
  let contentLists = $state<GeositeListOverview[] | null>(null);
  let query = $state('');
  ...
  let previewItems = $state<GeositeEntry[]>([]);
  ...
  async function loadCatalog(name: string) {
    ...
    clearPreview();
    try {
      const result = await api.getGeositeCatalog(name);
  ...
  async function searchContent(name: string, term: string) {
    ...
    error = '';
    try {
      const result = await api.getGeositeCatalog(name, term, 'content');
  ...
  function catalogError(error: unknown): string {
    if (error instanceof APIRequestError && error.code === 'geosite_cache_missing') {
  ...
  async function preview(list: GeositeListOverview, attr?: string) {
    if (!provider) return;
    ...
    previewError = '';
    try {
      const result = await api.getGeositeList(provider, list.name, { attr, limit: pageSize, offset: 0 });
  ...
  async function loadMore() {
    ...
    previewLoading = true;
    try {
      const result = await api.getGeositeList(provider, previewList, { attr: previewAttr || undefined, limit: pageSize, offset: previewItems.length });
      if (requestId !== previewRequest) return;
...
<PixelDrawer bind:open title={provider ? `${provider} 目录` : 'Geosite 目录'} icon={geositeIcon} width="880px" scrollable={false} onclose={close}>
  <div class="catalog-shell">
    <div class="catalog-toolbar">
      <PixelTabs id="geosite-search-mode" label="检索方式" items={[{ value: 'name', label: '名称' }, { value: 'content', label: '内容' }]} value={match} onchange={(value) => { match = value as 'name' | 'content'; }} />
      <input class="pixel-input" type="search" placeholder={match === 'content' ? '域名或规则内容…' : '搜索列表或变体…'} bind:value={query} spellcheck="false" />
    ...
    <div class="catalog-panes">
      <div class="catalog-lists" use:retroScroll>
        {#if loading}
          <div class="catalog-empty">读取目录…</div>
          ...
          {#each displayed as list (list.name)}
            <div class="geo-item" class:active={isActive(list.name)}>
              <div class="geo-row">
                ...
                <span class="geo-meta">
                  <span class="geo-count">{list.entries.toLocaleString()} 条</span>
      ...
      <div class="catalog-preview">
        ...
        {/if}
        <div class="preview-body" use:retroScroll>
          {#if !previewKey}
          ...
          {:else if previewItems.length === 0 && previewLoading}
            <div class="catalog-empty">读取规则…</div>
          {:else}
            <ul class="entry-list">
...
<style>
  ...
  .geo-tags { display: flex; flex-wrap: wrap; gap: 4px; justify-content: flex-end; }
  .geo-tag {
    padding: 1px 7px;
