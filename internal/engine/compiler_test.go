...
func TestCompileRuleFormatsOpsAndRendersCompletePipeline(t *testing.T) {
	...
	got := CompileRule(
		...
		testRegistry(t), nil, nil, nil, nil, testLogger(),
	)
...
func TestCompileRuleExpandsExplicitFormatsAndAppliesVariantOps(t *testing.T) {
	...
	got := CompileRule(context.Background(), rule, clients, NewFetcher(), NewPreprocessRunner(), testRegistry(t), nil, nil, nil, nil, testLogger())
...
func TestCompileRuleReadsLocalFileSource(t *testing.T) {
	...
	got := CompileRule(context.Background(), rule, nil, NewFetcher(), NewPreprocessRunner(), testRegistry(t), nil, nil, nil, config.NewLocalFileResolver(dataDir), testLogger())
...
func TestCompileRuleFetchesSourcesConcurrentlyWithinLimit(t *testing.T) {
	...
	go func() {
			...
			registry, nil, nil, nil, nil, testLogger(),
		)
	}()
